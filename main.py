...
# -----------------------
# Config (kept but webhook not required)
# -----------------------
class Config:
    def __init__(self):
        self.bot_token = os.getenv('BOT_TOKEN')
        # NOTE: WEBHOOK_URL used to be here; removed from logic — keep in .env if you want it but it's ignored.
        self.webhook_url = os.getenv('WEBHOOK_URL', None)

    def validate(self):
        if not self.bot_token:
            raise ValueError("BOT_TOKEN environment variable is required")
        # webhook_url no longer required
        return True

# -----------------------
# Storage paths (Railway PV / local fallback)
# -----------------------
...
# Structure:
# welcome_messages: { "<guild_id>": ["msg1", "msg2", ...], ... }
welcome_messages = load_json(WELCOME_FILE, {})

# sent_embeds: { "<message_id>": { "guild_id": <>, "channel_id": <> }, ... }
sent_embeds = load_json(SENT_EMBEDS_FILE, {})

# -----------------------
# IMPORTANT: Hard-coded welcome channels
# - You said you will supply the channel ID in code.
# - Fill in the mapping below: guild_id (int) -> channel_id (int)
# - Example: WELCOME_CHANNELS[123456789012345678] = 987654321098765432
# -----------------------
WELCOME_CHANNELS = {
    991908158274539681: 991943909565550643,
    1281605174556626994: 1410934962412195922,  # <- replace with your guild/channel IDs
}
...
# -----------------------
# Default messages fallback (if guild has no custom list)
# -----------------------
DEFAULT_MESSAGES = [
...
# -----------------------
# Bot (class & setup kept similar to yours)
# -----------------------
class WelcomeBot(commands.Bot):
    def __init__(self):
        ...
        super().__init__(command_prefix="!", intents=intents, help_command=None)  # Use any prefix like "!" or "/"

        self.config = Config()
        self.session = None

    async def setup_hook(self):
        # create session and sync slash commands
...
# -----------------------
# Welcome event (uses direct bot sending to the hard-coded channel)
# -----------------------
@bot.event
...
# -----------------------
# Slash commands: Welcome messages management (admin-only)
# -----------------------
...
async def list_welcome(interaction: discord.Interaction):
    ...
    # For long lists, you could send as a file; keep ephemeral
...
# -----------------------
# Test welcome (slash) — sends the welcome embed to the configured channel for this guild
# -----------------------
...
# - collects main options and opens modal for long fields
# -----------------------
...
async def create_embed_cmd(interaction: discord.Interaction,
                           ...
                           preview: bool = False):
    # Basic validation of URLs (simple) - do this quickly first
    bad = []
...
# -----------------------
# List sent embeds (for editing)
# -----------------------

# -----------------------
# Edit embed (admin-only)
# - Edit by message_id; supports new_title, new_description, append/remove fields not implemented here (basic edits)
# -----------------------
